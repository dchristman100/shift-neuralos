...
function SliderInput({ label, sublabel, value, onChange, min, max, step = 1, prefix = "", suffix = "", format }) {
...
function WoundBadge({ num, label, active }) {
  return (
    <div style={{
      display: "flex", alignItems: "center", gap: "10px",
      padding: "10px 20px", borderRadius: "999px",
      background: active ? "rgba(245,74,72,0.1)" : "rgba(255,255,255,0.03)",
      border: `1px solid ${active ? "rgba(245,74,72,0.35)" : "rgba(255,255,255,0.08)"}`,
      transition: "all 0.3s",
    }}>
      <div style={{
        width: 24, height: 24, borderRadius: "50%",
        background: active ? "linear-gradient(135deg,#F54A48,#FA982F)" : "rgba(255,255,255,0.08)",
        display: "flex", alignItems: "center", justifyContent: "center",
        fontFamily: "'Montserrat Alternates', sans-serif", fontSize: "11px", fontWeight: 800, color: "#fff",
        flexShrink: 0,
      }}>{num}</div>
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", color: active ? "#fff" : "rgba(255,255,255,0.35)", whiteSpace: "nowrap" }}>{label}</span>
    </div>
  );
}
...
export default function RevenueLeakCalculator({ onScrollToPackages }) {
  ...
  const calc = useMemo(() => {
    const lostLeads = (inboundLeads || 50) * ((missPct || 40) / 100);
    const lostJobs = lostLeads * ((closePct || 35) / 100);
    const wound1 = lostJobs * (avgTicket || 8000);

    const wastedSpend = (monthlySpend || 3000) * ((garbagePct || 35) / 100);
    const wastedLabor = ((hoursPerWeek || 5) * (hourlyRate || 35)) * 4.33;
    const wound2 = wastedSpend + wastedLabor;

    const qualLeads = (paidLeads || 50) * (1 - (garbagePct || 35) / 100);
    const costPerLead = qualLeads > 0 ? (monthlySpend || 3000) / qualLeads : null;
  ...
  const outputBox = (val, label, color = "#F54A48", big = false) => (
    <div style={{ textAlign: "center", padding: "20px 16px", background: "rgba(0,0,0,0.2)", borderRadius: "12px", border: `1px solid ${color}22` }}>
      <div style={{ fontFamily: "'Montserrat Alternates', sans-serif", fontSize: big ? "36px" : "28px", fontWeight: 900, color, marginBottom: "4px", lineHeight: 1 }}>{val}</div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(255,255,255,0.4)", lineHeight: 1.4 }}>{label}</div>
    </div>
  );
    ...
    <div style={{ fontFamily: "'Montserrat Alternates', sans-serif" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: "48px" }}>
        <div style={{
          ...
          letterSpacing: "0.2em", textTransform: "uppercase", color: "#FA982F",
          marginBottom: "20px",
        }}>
      ...
      <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", justifyContent: "center", marginBottom: "36px" }}>
        <WoundBadge num="1" label="Missed Calls" active={true} />
        <WoundBadge num="2" label="Garbage Leads" active={true} />
        <WoundBadge num="3" label="Total Leak + ROI" active={true} />
      </div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 40px" }}>
          <SliderInput label="Inbound leads per month" sublabel="All sources — calls, forms, texts, referrals" value={inboundLeads} onChange={setInboundLeads} min={5} max={300} step={5} suffix="" />
          ...
          <SliderInput label="Close rate on leads reached" sublabel="When you actually respond, how often do you close?" value={closePct} onChange={setClosePct} min={5} max={80} step={5} suffix="%" />
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginTop: "8px" }}>
          {outputBox(Math.round(calc.lostLeads) + " leads", "Lost per month", "#FA982F")}
          {outputBox(Math.round(calc.lostJobs * 10) / 10 + " jobs", "Lost per month", "#FA982F")}
          {outputBox(fmt(calc.wound1), "Estimated monthly revenue leak", "#F54A48", true)}
        </div>
        ...
        <div style={{ marginTop: "16px", padding: "14px 18px", background: "rgba(245,74,72,0.06)", borderLeft: "3px solid #F54A48", borderRadius: "0 8px 8px 0" }}>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "13px", color: "rgba(255,255,255,0.6)", lineHeight: 1.6, margin: 0, fontStyle: "italic" }}>
            "These leads already exist. ShiFt captures them — the AI responds in under 30 seconds, every time, 24/7."
          </p>
        </div>
      </div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 40px" }}>
          ...
          <SliderInput label="Hours/week chasing junk leads" sublabel="Self or team time on unqualified contacts" value={hoursPerWeek} onChange={setHoursPerWeek} min={1} max={40} step={1} suffix=" hrs" />
          ...
          <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
            {calc.costPerLead !== null
              ? <div style={{ textAlign: "center", padding: "20px 16px", background: "rgba(0,0,0,0.2)", borderRadius: "12px", border: "1px solid rgba(250,152,47,0.15)" }}>
                  <div style={{ fontFamily: "'Montserrat Alternates', sans-serif", fontSize: "28px", fontWeight: 900, color: "#FA982F", marginBottom: "4px" }}>{fmt(calc.costPerLead)}</div>
                  <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(255,255,255,0.4)" }}>Cost per qualified lead</div>
                </div>
              : <div style={{ textAlign: "center", padding: "20px", color: "rgba(255,255,255,0.3)", fontFamily: "'JetBrains Mono', monospace", fontSize: "12px" }}>N/A — enter leads above</div>
            }
          </div>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginTop: "8px" }}>
          {outputBox(fmt(calc.wastedSpend), "Wasted ad spend/mo", "#FA982F")}
          {outputBox(fmt(calc.wastedLabor), "Wasted labor/mo", "#FA982F")}
          {outputBox(fmt(calc.wound2), "Estimated garbage lead cost", "#F54A48", true)}
        </div>
        ...
        <div style={{ marginTop: "16px", padding: "14px 18px", background: "rgba(250,152,47,0.06)", borderLeft: "3px solid #FA982F", borderRadius: "0 8px 8px 0" }}>
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "13px", color: "rgba(255,255,255,0.6)", lineHeight: 1.6, margin: 0, fontStyle: "italic" }}>
            "ShiFt AI qualifies every lead in under 30 seconds before it ever touches your calendar — no wasted sales hours, no tire-kicker calls."
          </p>
        </div>
      </div>
      ...
      <div style={{ ...cardStyle, background: "linear-gradient(135deg, rgba(245,74,72,0.07), rgba(250,152,47,0.04))", border: "1px solid rgba(245,74,72,0.2)" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: "20px", marginTop: "16px", flexWrap: "wrap" }}>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "12px", color: "rgba(255,255,255,0.4)" }}>
              Wound 1: <strong style={{ color: "rgba(255,255,255,0.7)" }}>{fmt(calc.wound1)}</strong>
            </span>
            <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "12px", color: "rgba(255,255,255,0.4)" }}>
              Wound 2: <strong style={{ color: "rgba(255,255,255,0.7)" }}>{fmt(calc.wound2)}</strong>
            </span>
          </div>
        </div>
          ...
          <div style={{ display: "flex", gap: "10px", justifyContent: "center", flexWrap: "wrap" }}>
            {Object.values(PKG).map(p => (
              <button key={p.name} onClick={() => setSelectedPkg(p.name)} style={{
                padding: "10px 24px", borderRadius: "999px", border: `1px solid ${selectedPkg === p.name ? p.color : "rgba(255,255,255,0.12)"}`,
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "28px" }}>
          <div style={{ textAlign: "center", padding: "24px 16px", background: "rgba(0,0,0,0.25)", borderRadius: "14px", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ fontFamily: "'Montserrat Alternates', sans-serif", fontSize: "28px", fontWeight: 900, color: "rgba(255,255,255,0.6)", marginBottom: "6px" }}>{fmt(PKG[selectedPkg].cost)}</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(255,255,255,0.35)" }}>Monthly engine cost</div>
          </div>
          <div style={{ textAlign: "center", padding: "24px 16px", background: calc.netReturn >= 0 ? "rgba(72,187,120,0.08)" : "rgba(245,74,72,0.08)", borderRadius: "14px", border: `1px solid ${calc.netReturn >= 0 ? "rgba(72,187,120,0.2)" : "rgba(245,74,72,0.2)"}` }}>
            <div style={{ fontFamily: "'Montserrat Alternates', sans-serif", fontSize: "28px", fontWeight: 900, color: calc.netReturn >= 0 ? "#48BB78" : "#F54A48", marginBottom: "6px" }}>{fmt(calc.netReturn)}</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(255,255,255,0.35)" }}>Estimated net monthly return</div>
          </div>
          <div style={{ textAlign: "center", padding: "24px 16px", background: "rgba(250,152,47,0.08)", borderRadius: "14px", border: "1px solid rgba(250,152,47,0.2)" }}>
            <div style={{ fontFamily: "'Montserrat Alternates', sans-serif", fontSize: "28px", fontWeight: 900, color: "#FA982F", marginBottom: "6px" }}>{Math.round(calc.roiMultiple)}×</div>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", textTransform: "uppercase", letterSpacing: "0.12em", color: "rgba(255,255,255,0.35)" }}>ROI multiplier</div>
          </div>
        </div>

        {/* Guarantee */}
        <div style={{ background: "rgba(0,0,0,0.2)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "20px 24px", marginBottom: "24px", textAlign: "center" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <button onClick={onScrollToPackages} style={{
            ...
            boxShadow: "0 4px 24px rgba(245,74,72,0.3)", transition: "all 0.3s",
          }}>
          ...
          <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", color: "rgba(255,255,255,0.25)", marginTop: "12px", textTransform: "uppercase", letterSpacing: "0.1em" }}>
            All outputs are estimated based on your inputs
          </div>
        </div>
      </div>
    </div>
